...
nohup ssh -o IgnoreUnknown=TofuCreatedThisTunnel \
  ...
  %{ if ssh_bastion_host != null ~}-o ProxyCommand="ssh -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null -i ${ssh_bastion_key_path} -W %h:%p ${ssh_bastion_user}@${ssh_bastion_host}"\%{ endif ~}
  ${ssh_user}@${ssh_bastion_host != null ? private_ip : public_ip} >/dev/null 2>&1 &
