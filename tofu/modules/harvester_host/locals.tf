...
EOT
  ...
  authorized_keys_userdata = indent(2, replace(trimspace(yamlencode({ssh_authorized_keys: local.public_keys})), "\"", ""))
...
# TODO: try adding user in cloud-init config
  ...
  private_network_interfaces = { for network in harvester_virtualmachine.this.network_interface[*] : network.name => {
    interface_name = network.interface_name
    ip_address     = network.ip_address
    } if !local.networks_map[network.name].public
  }
  public_network_interfaces = { for network in harvester_virtualmachine.this.network_interface[*] : network.name => {
    interface_name = network.interface_name
    ip_address     = network.ip_address
    } if local.networks_map[network.name].public
  }
