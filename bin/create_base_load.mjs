...
for (const [name, downstream] of downstreams) {
    k6_run(tester,
        { BASE_URL: `https://${downstream["private_name"]}:6443`, KUBECONFIG: downstream["kubeconfig"], CONTEXT: downstream["context"], COUNT: CONFIG_MAP_COUNT},
        {commit: commit, cluster: name, test: "create_load.mjs"},
        "k6/create_config_maps.js", true
    )
}
